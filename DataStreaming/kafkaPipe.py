#modularized code to consume data from the kafka stream
...
import requests
import pickle
import DataStreaming.Offsets as Offsets
    ...
    try:
        if (mpgStr in msgVal):
            if (msgSplit[1].isdigit() and 0<int(msgSplit[1]) and int(msgSplit[1])<1000001): #check id
                msgSplit2 = msgSplit[2].split("/",5) # check mpg and moviename
                if (msgSplit2[4][:-4].isdigit() and int(msgSplit2[4][:-4])>0): #check mpg
                    if(msgSplit2[3] in movieList):
                        mpgWr.writerow(msgSplit)
                        return
                    elif(requests.get(URL + msgSplit2[3]).status_code == 200):
                        movieList.append(msgSplit2[3])
                        mpgWr.writerow(msgSplit)
                        return
                    return
        elif (recStr in msgVal):
            msgList = msgSplit[0:4]
            msgList.extend(msgSplit[4].rsplit(",",1))
            recsWr.writerow(msgList)
            return
        #elif (rateStr in msgVal):
        #    if (msgSplit[1].isdigit() and 0<int(msgSplit[1]) and int(msgSplit[1])<1000001): #check id
...
def writeOffsets(pOff):
    with open("DataStreaming/Offsets.py", "w") as myfile:
